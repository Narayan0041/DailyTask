...
const StudentListPresenty = (props) => {
  let navigate =useNavigate();
  const ClickTheProfile = () => {
    navigate(`/TeacherAttendance/${props.StudentName}`);
    ...
    if (storageKey) {
      ...
      StudentAttendancePresent.push(`${props.StudentName}-Present`);
    ...
    StudentAttendanceAbsent.push(`${props.StudentName}-Absent`);
    localStorage.setItem("GetResultInMapAbsent", JSON.stringify(StudentAttendanceAbsent));
  };

  const HandleClickPresent = (event, key) => {
    event.stopPropagation();
    alert(`Marked present for key: ${key}`);
    const storageKey = `${props.StudentName}-Present`;
    let StudentAttendancePresent = localStorage.getItem(storageKey);
    StudentAttendancePresent = StudentAttendancePresent ? JSON.parse(StudentAttendancePresent) : [];
  ...
  const HandleClickAbsent = (event, key) => {
    event.stopPropagation();
    alert(`Marked absent for key: ${key}`);
    const storageKey = `${props.StudentName}-Absent`;
    let StudentAttendancePresent = localStorage.getItem(storageKey);
    StudentAttendancePresent = StudentAttendancePresent ? JSON.parse(StudentAttendancePresent) : [];
  ...
  return (
    <div className="StudentListSection">
      <div className="StudentListContainer">
        <div className="StudentBoxss" onClick={() => { ClickTheProfile(`${props.StudentName}-Present`) }}>
          ...
          <div className='Attendance'>
            <div className='AttendanceButton' onClick={(event) => { HandleClickPresent(event, "Present") }}>P</div>
            <div className='AttendanceButton' onClick={(event) => { HandleClickAbsent(event, "Absent") }}>A</div>
          </div>
        </div>
...
export default StudentListPresenty;
